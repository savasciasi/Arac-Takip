...
from .database import current_brand, get_connection, storage_path, table_name
...
STATE_FILE = state_file("schema_state.json")
SEED_VERSION = 1
...
def _mark_seeded() -> None:
    state = _load_state()
    entry = state.setdefault(_state_key(), {})
    entry["seed_version"] = SEED_VERSION
    entry["seed_updated_at"] = datetime.utcnow().isoformat()
    _save_state(state)
...
def run() -> None:
    ...
    if entry.get("seed_version") == SEED_VERSION:
        return
