...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self._needs_refresh: dict[str, bool] = {}
    ...
    def _navigate(self, key: str) -> None:
        ...
        self._ensure_refreshed(key)

    def _ensure_refreshed(self, key: str) -> None:
        """Refresh the requested page if its data is marked as stale."""

        if not self._needs_refresh.get(key, True):
            return
    ...
    def _refresh_pages(self, immediate: bool = True) -> None:
        """Mark pages as stale and optionally refresh the active one immediately."""

        for key in self._pages:
            self._needs_refresh[key] = True
        if immediate and self._current_key:
            self._ensure_refreshed(self._current_key)
